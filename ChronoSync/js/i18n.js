(function () {
  if (window.ChronoSyncI18n) return;

  var copies = {
    en: {
      subtitle:       'Align and compare photos across time',
      introTitle:     'What is ChronoSync?',
      uploadTitle:    'Photos',
      addPhotos:      'Browse files',
      dropHint:       'Drag & drop photos here or',
      referenceLabel: 'Reference image',
      alignBtn:       'Align photos',
      clearBtn:       'Clear all',
      statusEmpty:    'Add at least two photos to begin.',
      statusOne:      'Add at least one more photo.',
      statusReady:    ' photos loaded. Click "Align photos" to process.',
      loadingEngine:  'Loading alignment engine…',
      resizing:       'Resizing images…',
      detecting:      'Detecting and matching features…',
      applyingAlign:  'Applying alignment…',
      alignedStatus:  ' photos aligned.',
      previewTitle:   'Collage preview',
      showKps:        'Show keypoints',
      showGrid:       'Show grid',
      trimBlack:      'Trim black',
      fillGaps:       'Fill gaps',
      mosaicLayout:   'Layout',
      mosaicCascade:  'Cascade',
      mosaicGrid:     'Grid',
      mosaicComposite:'Composite',
      mosaicOrient:   'Orientation',
      orientAuto:     'Auto',
      orientSide:     'Side by side',
      orientStacked:  'Stacked',
      mosaicColumns:  'Columns',
      mosaicPattern:  'Pattern',
      patStripes:     'Stripes',
      patBlocks:      'Blocks',
      patRadial:      'Radial',
      mosaicBands:    'Bands/img',
      mosaicAngle:    'Angle',
      mosaicBlocks:   'Blocks',
      mosaicSectors:  'Sectors',
      downloadBtn:    'Download collage',
      outputEdits:    'Output edits',
      resetEdits:     'Reset edits',
      editHint:       'Basic non-destructive adjustments before download.',
      brightness:     'Brightness',
      contrast:       'Contrast',
      saturation:     'Saturation',
      temperature:    'Temperature',
      tint:           'Tint',
      gamma:          'Gamma',
      dateLabelTitle: 'Date label',
      chooseColor:    'Choose a color',
      reference:      'reference',
      inliers:        'inliers',
      convertingHeic: 'Converting HEIC…',
      loadError:      'Could not load',
      labelSettings:  'Label settings',
      labelFont:      'Font',
      labelSize:      'Size',
      labelPos:       'Position',
      labelColor:     'Text color',
      labelBgOpacity: 'Bg color',
      labelMargin:    'Margin'
    },
    es: {
      subtitle:       'Alinea y compara fotos a lo largo del tiempo',
      introTitle:     '¿Qué es ChronoSync?',
      uploadTitle:    'Fotos',
      addPhotos:      'Seleccionar archivos',
      dropHint:       'Arrastra fotos aquí o',
      referenceLabel: 'Imagen de referencia',
      alignBtn:       'Alinear fotos',
      clearBtn:       'Limpiar todo',
      statusEmpty:    'Añade al menos dos fotos para empezar.',
      statusOne:      'Añade al menos una foto más.',
      statusReady:    ' fotos cargadas. Haz clic en “Alinear fotos” para procesar.',
      loadingEngine:  'Cargando motor de alineación…',
      resizing:       'Redimensionando imágenes…',
      detecting:      'Detectando y emparejando puntos…',
      applyingAlign:  'Aplicando alineación…',
      alignedStatus:  ' fotos alineadas.',
      previewTitle:   'Vista previa del collage',
      showKps:        'Mostrar puntos clave',
      showGrid:       'Mostrar cuadrícula',
      trimBlack:      'Recortar negro',
      fillGaps:       'Rellenar huecos',
      mosaicLayout:   'Disposición',
      mosaicCascade:  'Cascada',
      mosaicGrid:     'Cuadrícula',
      mosaicComposite:'Composición',
      mosaicOrient:   'Orientación',
      orientAuto:     'Auto',
      orientSide:     'Lado a lado',
      orientStacked:  'Apilado',
      mosaicColumns:  'Columnas',
      mosaicPattern:  'Patrón',
      patStripes:     'Franjas',
      patBlocks:      'Bloques',
      patRadial:      'Radial',
      mosaicBands:    'Bandas/img',
      mosaicAngle:    'Ángulo',
      mosaicBlocks:   'Bloques',
      mosaicSectors:  'Sectores',
      downloadBtn:    'Descargar collage',
      outputEdits:    'Ajustes de salida',
      resetEdits:     'Restablecer ajustes',
      editHint:       'Ajustes básicos no destructivos antes de descargar.',
      brightness:     'Brillo',
      contrast:       'Contraste',
      saturation:     'Saturación',
      temperature:    'Temperatura',
      tint:           'Tinte',
      gamma:          'Gamma',
      dateLabelTitle: 'Etiqueta de fecha',
      chooseColor:    'Elegir un color',
      reference:      'referencia',
      inliers:        'inliers',
      convertingHeic: 'Convirtiendo HEIC…',
      loadError:      'No se pudo cargar',
      labelSettings:  'Formato de etiquetas',
      labelFont:      'Fuente',
      labelSize:      'Tamaño',
      labelPos:       'Posición',
      labelColor:     'Color texto',
      labelBgOpacity: 'Color fondo',
      labelMargin:    'Margen'
    }
  };

  window.ChronoSyncI18n = {
    getCopy: function (lang) { return copies[lang] || copies.en; }
  };
})();
